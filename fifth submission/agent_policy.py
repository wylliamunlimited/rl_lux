...
class AgentPolicy(AgentWithModel):
    ...
    def get_reward(self, game, is_game_finished, is_new_turn, is_game_error):
        ...
        # Penalize for having cart outside during the night
        rewards["rew/r_night_cart_out"] = 0
        if (game.is_night()): 
            rewards["rew/r_night_cart_out"] = self.avg_distance_from_cities("cart", game, self.team) * (-0.1)

        # Penalize for far distance between cart and worker
        rewards["rew/r_cart_to_worker"] = self.count_min_distance_larger_than_expected(game, self.team, 1) * (-0.01)
        ...
        rewards["rew/r_research_point"] = ( (research_point - self.research_point_last) / 500 )
        self.research_point_last = research_point

        # Reward collecting wood
        wood_collected = game.stats["teamStats"][self.team]["resourcesCollected"]["wood"]
        rewards["rew/r_wood_collected"] = ( (wood_collected - self.wood_collected_last) / 100 )
        self.wood_collected_last = wood_collected

        # Reward collecting coal
        coal_collected = game.stats["teamStats"][self.team]["resourcesCollected"]["coal"]
        rewards["rew/r_coal_collected"] = ( (coal_collected - self.coal_collected_last) / 10 )
        ...
        rewards["rew/r_uranium_collected"] = ( (uranium_collected - self.uranium_collected_last) )
        self.uranium_collected_last = uranium_collected
        
        # Reward Generating fuel
        fuel_generated = game.stats["teamStats"][self.team]["fuelGenerated"]
        rewards["rew/r_fuel_generated"] = ( (fuel_generated - self.fuel_generated_last) / 100 )
        self.fuel_generated_last = fuel_generated
    ...
    # TODO: use CLOSEST_NODE to get the distance between each unit and every cities --> Get the min distance and average --> return avg distance
    def avg_distance_from_cities(self, worker_type, game, team):
        distance = {}
        current_state = game.state
        current_cities = game.cities
        
        for unit in current_state["teamStates"][team]["units"].values():
            if ((worker_type == "worker" and unit.is_cart()) or (worker_type=="cart" and unit.is_worker())):
                continue
            unit_pos_x, unit_pos_y = unit.pos.x, unit.pos.y
            distance[unit.id] = []
            for city in current_cities.values():
                for cell in city.city_cells:
                    delta = np.sqrt((unit_pos_x - cell.pos.x) ** 2 + (unit_pos_y - cell.pos.y) ** 2)
                    distance[unit.id].append(delta)

        for unit, distances in distance.items():
            if distances:
        ...
        return sum(distance.values()) / len(distance) if len(distance) > 0 else -10

    def count_min_distance_larger_than_expected(self, game, team, distance_threshold):
        distance = {} # worker: {carts: distances}
        current_state = game.state

        for unit in current_state["teamStates"][team]["units"].values():
            if (unit.is_worker()):
                distance[unit.id] = []
                worker_pos_x, worker_pos_y = unit.pos.x, unit.pos.y
                for subunit in current_state["teamStates"][team]["units"].values():
                    if (subunit.is_cart()):
                        cart_pos_x, cart_pos_y = subunit.pos.x, subunit.pos.y
                        distance[unit.id].append(np.sqrt((worker_pos_x - cart_pos_x) ** 2 + (worker_pos_y - cart_pos_y) ** 2))
        ...
        return sum(1 for value in distance.values() if value > distance_threshold or value < 0)

    # def get_distance_to_resources(self, resource_map, worker_type="worker"):
    #     """
    #         take resource_map and calculate distance between each unit and resources
    #         worker_type: either worker or cart
    #         return dict(unit, [distance])
    #     """
    #     distances = {}
